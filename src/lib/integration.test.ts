...
describe('Integration Tests', () => {
  ...
  describe('Allocation Pipeline Integration', () => {
    it('should properly allocate funds based on financial phase', () => {
      ...
      const totalAllocated = allocations.debtPayment + allocations.savingsContribution + allocations.investmentContribution;
      assert.strictEqual(totalAllocated, testData.freeCapital);
      
      // Step 2: Calculate actual debt payment
      const debtPayment = calculateDebtPayment(testData.currentDebt, allocations.debtPayment);
      
      assert(debtPayment <= testData.currentDebt, 'Cannot pay more than debt owed');
      assert(debtPayment <= allocations.debtPayment, 'Cannot pay more than allocated');
      assert.strictEqual(debtPayment, allocations.debtPayment); // In this case, allocation < debt
    });
  });
  ...
  describe('Wealth Simulation Integration', () => {
    it('should run complete wealth simulation with realistic data', () => {
      ...
      const retirementYear = simulation.find((year: any) => year?.age === 65);
      if (retirementYear) {
