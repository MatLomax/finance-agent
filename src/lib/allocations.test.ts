...
describe('calculateAllocationAmounts', () => {
  it('should calculate allocations with standard percentages', () => {
    ...
    assert.strictEqual(result.debtPayment, 8000);
    assert.strictEqual(result.savingsContribution, 1000);
    assert.strictEqual(result.investmentContribution, 1000);
  });
  
  it('should handle zero free capital', () => {
    const result = calculateAllocationAmounts(0, 0.8, 0.1, 0.1);
    
    assert.strictEqual(result.debtPayment, 0);
    assert.strictEqual(result.savingsContribution, 0);
    assert.strictEqual(result.investmentContribution, 0);
  });
  
  it('should handle 100% debt allocation', () => {
    const result = calculateAllocationAmounts(5000, 1.0, 0.0, 0.0);
    
    assert.strictEqual(result.debtPayment, 5000);
    assert.strictEqual(result.savingsContribution, 0);
    assert.strictEqual(result.investmentContribution, 0);
  });
  ...
  it('should handle 100% investment allocation', () => {
    ...
    assert.strictEqual(result.debtPayment, 0);
    assert.strictEqual(result.savingsContribution, 0);
    assert.strictEqual(result.investmentContribution, 3000);
  });
  
  it('should handle equal allocations', () => {
    const result = calculateAllocationAmounts(3000, 0.333, 0.333, 0.334);
    
    assert.strictEqual(result.debtPayment, 999);
    assert.strictEqual(result.savingsContribution, 999);
    // Use Math.round to handle floating point precision
    assert.strictEqual(Math.round(result.investmentContribution), 1002);
  });
  
  it('should handle negative free capital', () => {
    const result = calculateAllocationAmounts(-2000, 0.5, 0.3, 0.2);
    
    assert.strictEqual(result.debtPayment, -1000);
    assert.strictEqual(result.savingsContribution, -600);
    assert.strictEqual(result.investmentContribution, -400);
  });
  ...
  it('should handle small capital amounts', () => {
    ...
    assert.strictEqual(result.debtPayment, 0.5);
    assert.strictEqual(result.savingsContribution, 0.3);
    assert.strictEqual(result.investmentContribution, 0.2);
  });
  
  it('should handle large capital amounts', () => {
    const result = calculateAllocationAmounts(1000000, 0.6, 0.2, 0.2);
    
    assert.strictEqual(result.debtPayment, 600000);
    assert.strictEqual(result.savingsContribution, 200000);
    assert.strictEqual(result.investmentContribution, 200000);
  });
  
  it('should preserve precision for decimal allocations', () => {
    const result = calculateAllocationAmounts(7500, 0.666, 0.167, 0.167);
    
    assert.strictEqual(result.debtPayment, 4995);
    assert.strictEqual(result.savingsContribution, 1252.5);
    assert.strictEqual(result.investmentContribution, 1252.5);
  });
});
