 ...
 * 
 */
export function findOptimalRetirementAge(input: any): any {
  const { 
    currentAge, 
    lifespan, 
    monthlyExpenses,
 ...
 */
function evaluateRetirementAge(input: any, testAge: number, annualExpenses: number, currentBest: number): any {
  ...
  const retirementYear = simulation.find((year: any) => year?.age === testAge);
