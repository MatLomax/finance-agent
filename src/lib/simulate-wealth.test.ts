...
describe('Allocation Amount Calculations', () => {
  ...
  test('should throw error for unknown phase', () => {
    assert.throws(() => {
      calculateAllocationAmounts(1000, 'unknown' as any, testAllocations, 0);
...
describe('Wealth Trajectory Simulation', () => {
  ...
  test('should transition phases as debt is paid and emergency fund built', () => {
    ...
    const debtFreeYear = simulation.find((year: any) => year?.debt === 0 && year?.age > quickDebtInput.currentAge);
    ...
    const emergencyPhaseYear = simulation.find((year: any) => 
      year?.debt === 0 && 
  ...
  test('should handle retirement phase correctly', () => {
    ...
    const retiredYear = simulation.find((year: any) => year?.isRetired);
...
describe('Optimal Retirement Age Finding', () => {
  ...
  test('should return simulation matching the optimal age', () => {
    ...
    const retirementYear = result.simulation.find((year: any) => year?.age === result.age);
