 ...
 */
export function formatGrowth(current: number, previous: number | null, showPercentage: any = true, showColors: any = true): string {
 ...
 */
export function formatDelta(current: number, previous: number | null, minThreshold: any = 1, currency: any = '€'): string {
  validateNumber(current, 'current');
  if (previous !== null) {
    validateNumber(previous, 'previous');
  }
