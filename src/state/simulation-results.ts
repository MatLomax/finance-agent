...
function generateCacheKey(input: any): string {
  // Create a normalized copy for consistent cache keys
  ...
  // Use a simple hash for manageable cache keys
 ...
 * 
 */
export function getCachedSimulation(input: any): any {
 ...
 * 
 */
export function setCachedSimulation(input: any, result: any): void {
  try {
    const cacheKey = generateCacheKey(input);
    ...
    try {
      ...
      localStorage.setItem(`sim_${cacheKey}`, JSON.stringify(result));
    } catch (storageError) {
      // localStorage may be full or unavailable
      console.warn('Could not persist simulation to localStorage:', storageError);
    }
  } catch (error) {
 ...
 * 
 */
export function invalidateSimulationCache(input: any): string {
  const cacheKey = generateCacheKey(input);
 ...
 */
export function clearSimulationCache(): string {
 ...
 * 
 */
export function getSimulationCacheStats(): string {
  return {
